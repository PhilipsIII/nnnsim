 ...
 *
 *  nnn-icn-name.cc is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU Affero General Public License as published by
 *  the Free Software Foundation, either version 3 of the License, or
 *  (at your option) any later version.
 *
 *  nnn-icn-name.cc is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU Affero General Public License for more details.
 *
 *  You should have received a copy of the GNU Affero General Public License
 *  along with nnn-icn-name.cc. If not, see <http://www.gnu.org/licenses/>.
 *
...
{ 
  ...
  if (firstSlash == end)
    {
      BOOST_THROW_EXCEPTION (nnn::error::ICNName ()
                             << nnn::error::msg ("Name should include at least one slash (did you forget to specify initial /?)"));
    }
  ...
  if (firstSlash != i)
    {
      ...
      if (*schema.rbegin () != ':')
        {
          BOOST_THROW_EXCEPTION (nnn::error::ICNName ()
                                 << nnn::error::msg ("First component of the name does not start with a slash (did you forget to specify initial /?)"));
        }
      ...
      if (!boost::iequals (schema, "ccnx:") &&
        ...
        {
          BOOST_THROW_EXCEPTION (nnn::error::ICNName ()
                                 << nnn::error::msg ("URI schema is not supported (only ccnx: or ndn: is allowed)")
                                 << nnn::error::msg (schema));
        }
    }
...
Name::get (int index) const
{
  ...
  if (static_cast<unsigned int> (index) >= size ())
    {
      BOOST_THROW_EXCEPTION (nnn::error::ICNName ()
                             << nnn::error::msg ("Index out of range")
                             << nnn::error::pos (index));
    }
...
Name::getSubName (size_t pos/* = 0*/, size_t len/* = Name::npos*/) const
{
  ...
  if (pos + len > size ())
    {
      BOOST_THROW_EXCEPTION (nnn::error::ICNName ()
                             << nnn::error::msg ("getSubName parameter out of range")
                             << nnn::error::pos (pos)
                             << nnn::error::pos (len));
    }
