...
namespace ns3
{
  namespace nnn
  {
	...
	bool operator () (const Ptr<Face> &lhs, const Ptr<Face> &rhs) const
	{
      ...
      virtual void
      ProcessInterest (Ptr<NNNPDU> pdu, Ptr<Face> face, Ptr<Interest> interest);

      virtual void
      ProcessData (Ptr<NNNPDU> pdu, Ptr<Face> face, Ptr<Data> data);
    ...
    protected:
      ...
      typedef void (* NNNAddrTracedCallback)
	  (void);
